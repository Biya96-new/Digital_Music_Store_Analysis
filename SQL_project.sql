...
Festival in the city we made most money.Write a query that has one city that has 
the highest sum of invoice totals.Return both city name and sum of all invoice
totals.
...
Q2: We want to find out the most popular music Genrefor each country.We determine 
The most popular genre as the genre with the highest amount of purchases.Write a query
that returns each country along with top genre.For countries where the maximum number 
...
Q3. Write a query that determines the cutomer that has spent most on music for each
country.Write a query that returns the country along with top customers and how much
they spent.For countries where top amount spent is shared,provide all customers who 
spent this amount.
